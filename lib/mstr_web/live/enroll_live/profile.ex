defmodule MstrWeb.EnrollLive.Profile do
  ...
  # generated by claude (slight refactoring)
  ...
  # generated by claude
  defp validate_no_duplicate_tracks(changeset) do
    track_url_fields =
      changeset.data.__struct__.__schema__(:fields)
      |> Enum.filter(&(to_string(&1) =~ ~r/^track_url_/))

    track_ids_with_fields =
      track_url_fields
      |> Enum.map(fn field ->
        case get_change(changeset, field) do
          ...
          url ->
            try do
              {field, SSpotify.extract_track_id!(url)}
            rescue
              _ -> nil
            end
        end
    ...
    case duplicates do
      ...
      duplicate_ids ->
        Enum.reduce(track_ids_with_fields, changeset, fn {field, track_id}, acc ->
          if track_id in duplicate_ids do
            add_error(acc, field, "track must be unique")
          else
            acc
          end
        end)
